        ...
        if(!$erro){
            if(isset($_POST["salvar"])){
                ...
                if($target_file != $_SERVER['DOCUMENT_ROOT']."imagens/"){
                    ...
                    $sql = "UPDATE usuario SET usuario.nome = '$nome', usuario.nasc = '$nascimento', usuario.foto = '$foto', usuario.telefone = '$telefone', usuario.Senha = '$senha',
                    usuario.email = '$email'
                            WHERE usuario.CPF = '$cpf';";
                    move_uploaded_file($_FILES["foto"]["tmp_name"], $relative_file);
                }else {
                    $sql = "UPDATE usuario SET usuario.nome = '$nome', usuario.nasc = '$nascimento', usuario.telefone = '$telefone', usuario.Senha = '$senha',
                    usuario.email = '$email'
                            WHERE usuario.CPF = '$cpf';";
                }
    ...
    <body>
        <div class="wrapper">
            <div class="header">
                ...
                <button style="margin-left: 65%;" onclick="window.location.href='dadosCadastraisCliente.php'" class="btn btn2" type="button" name="button">Dados cadastrais cliente</button>
                <button onclick="window.location.href='historicoAcessosCliente.php'" class="btn btn2" type="button" name="button">Histórico de acessos cliente</button>
                <button onclick="window.location.href='telaInicialAcesso.php'" class="btn btn2" type="button" name="button">Logout</button>
            </div>
                    ...
                    <fieldset>
                        ...
                        <div class="form-group col-xs-3 <?php if(!empty($erro_departamento)){echo "has-error";}?>">
                            ...
                            <?php
                                ...
                                mysqli_close($conn);
                            ?>
                            <select disabled style="color:black;" class="form-control input-sm" name="departamento" id="departamento">
                        ...
                        <div class="form-group col-xs-3 <?php if(!empty($erro_cargo)){echo "has-error";}?>">
                            ...
                            <?php
                                ...
                                mysqli_close($conn);
                            ?>
                            <select disabled style="color:black;" class="form-control input-sm" name="cargo" id="cargo">
                                <option>Selecione...</option>
                                <?php while($dep = mysqli_fetch_array($query)) { ?>
                        ...
                        <div class="form-group col-xs-3 <?php if(!empty($erro_filial)){echo "has-error";}?>">
                            ...
                            <?php
                                ...
                                mysqli_close($conn);
                            ?>
                            <select disabled style="color:black;" class="form-control input-sm" name="filial" id="filial">
                                <option>Selecione...</option>
                                <?php while($dep = mysqli_fetch_array($query)) { ?>
        ...
        <script src="js/dadosCadastrais2.js"></script>
    </body>
</html>
